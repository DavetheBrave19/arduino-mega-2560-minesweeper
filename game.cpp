...
bool withinGrid(int column, int row) {
  return (column >= 0 && column < COLUMNS_OF_BLOCK && row >= 0 && row < ROWS_OF_BLOCK);
}
...
bool isAdjacent(int x, int y) {
  return (abs(x - posX) <= 1 && abs(y - posY) <= 1);
}
...
void moveCursor() {
  ...
  if(joySelect == LOW) {
    // randomizes the location of the mines after first block selection
    if(!minesGenerated) {
